...
Some interesting trends to note among the checked claims: - Virtually no True statements made by Republicans on CNN - Higher proportion of Pants on Fire claims made by Democrats on FOX - Significantly more False claims made by Republicans on NBC - Virtually no Pants on Fire statements made by Republicans on Regional Stations
...
loc_tags %>%
  filter(location == "Speech/Remarks", 
         location.extra == "Press Conference" | location.extra == "Press Briefing") %>%
 ggplot(aes(x = claimant_party, fill = textualRating)) +
  geom_bar(position = "fill") +
  facet_wrap(~ location.extra) +
  labs(
...
loc_tags %>%
  filter(location == "Speech/Remarks", 
         location.extra != "Press Conference",
         location.extra != "Press Briefing",
 ...
 ggplot(aes(x = claimant_party, fill = textualRating)) +
  ...
  labs(
    ...
    subtitle = "Not listed as Press or Campaign Events"
  ) +
  scale_fill_manual(values = truenesscolors)
```
