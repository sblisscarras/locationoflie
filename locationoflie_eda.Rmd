...
This data appears to indicate that Facebook has a higher proportion of falsehoods than Twitter, among all checked statements on PolitiFact. Of all statements on Facebook, 50.1% of statements were found to be Mostly False, False, or Pants on Fire, compared to 39.8% of the same on Twitter. Twitter also had a higher proportion of True statements checked, 16.3% to Facebook's 12.9%, about 1.26x higher. There are 2x as many Pants on Fire claims on Facebook when compared to Twitter.
...
FOX also had significantly less True statements recorded, and significantly more Pants on Fire claims. Compared to the next most extreme value, FOX had 0.63x as many True statements as NBC, and 3.19x as many Pants on Fire statements as CNN. Compared to Regional Stations, which has the lowest proportion of Pants on Fire claims, FOX news has 13.2x as many Pants on Fire claims.
...
cnn_prop_dems <- loc_tags %>%
  filter(location == "TV Appearance",
         ...
         claimant_party == "Democratic") %>%
  group_by(textualRating) %>%
  summarize(prop = n()/tv_totals_dems[1])
...
fox_prop_dems <- loc_tags %>%
  filter(location == "TV Appearance",
         ...
         claimant_party == "Democratic") %>%
  group_by(textualRating) %>%
  summarize(prop = n()/tv_totals_dems[2])
...
msnbc_prop_dems <- loc_tags %>%
  filter(location == "TV Appearance",
         ...
         claimant_party == "Democratic") %>%
  group_by(textualRating) %>%
  summarize(prop = n()/tv_totals_dems[3])
...
nbc_prop_dems <- loc_tags %>%
  filter(location == "TV Appearance",
         ...
         claimant_party == "Democratic") %>%
  group_by(textualRating) %>%
  summarize(prop = n()/tv_totals_dems[4])
...
rs_prop_dems <- loc_tags %>%
  filter(location == "TV Appearance",
         ...
         claimant_party == "Democratic") %>%
  group_by(textualRating) %>%
  summarize(prop = n()/tv_totals_dems[5])
rs_prop_dems$channel <- "Regional Station"
rs_prop_dems$party <- "Democratic"

tv_props_dems <- rbind(cnn_prop_dems, fox_prop_dems, msnbc_prop_dems, nbc_prop_dems, rs_prop_dems)

tv_totals_reps <- loc_tags %>%
  filter(location == "TV Appearance",
           location.extra == "CNN" | 
           location.extra == "FOX" | 
           location.extra == "MSNBC" | 
...
cnn_prop_reps <- loc_tags %>%
  filter(location == "TV Appearance",
         ...
         claimant_party == "Republican") %>%
  group_by(textualRating) %>%
  summarize(prop = n()/tv_totals_reps[1])
...
fox_prop_reps <- loc_tags %>%
  filter(location == "TV Appearance",
         ...
         claimant_party == "Republican") %>%
  group_by(textualRating) %>%
  summarize(prop = n()/tv_totals_reps[2])
...
msnbc_prop_reps <- loc_tags %>%
  filter(location == "TV Appearance",
         ...
         claimant_party == "Republican") %>%
  group_by(textualRating) %>%
  summarize(prop = n()/tv_totals_reps[3])
...
nbc_prop_reps <- loc_tags %>%
  filter(location == "TV Appearance",
         ...
         claimant_party == "Republican") %>%
  group_by(textualRating) %>%
  summarize(prop = n()/tv_totals_reps[4])
...
rs_prop_reps <- loc_tags %>%
  filter(location == "TV Appearance",
         ...
         claimant_party == "Republican") %>%
  group_by(textualRating) %>%
  summarize(prop = n()/tv_totals_reps[5])
rs_prop_reps$channel <- "Regional Station"
rs_prop_reps$party <- "Republican"

tv_props_reps <- rbind(cnn_prop_reps, fox_prop_reps, msnbc_prop_reps, nbc_prop_reps, rs_prop_reps)
...
tv_props_byparty %>%
  ...
  kable(caption = "Claim Rating Proportions by Party and TV Channel", digits = 3)
```
...
\- Highest proportion of Pants on Fire claims made by Democrats on FOX (25.0% of Democratic claims checked on FOX)

\- About equal proportion of False-ish statements on MSNBC (38.1% for Democrats, 38.9% for Republicans)

\- Republicans appear to lie most on NBC, with 68.9% of claims checked as False-ish

\- NBC also has the largest difference between True-ish statements by party, 67.9% Democratic claims were checked as True-ish whereas only 31.0% of Republican claims were checked as True-ish. This is a 36.9% difference.
...
loc_tags %>%
  ...
  group_by(location.extra) %>%
  summarize(count = n()) %>%
  kable(caption = "Type of Debate, count")
...
loc_tags %>%
 ...
 ggplot(aes(x = location.extra, fill = textualRating)) +
  ...
  labs(
    ...
    y = "Rating Distribution",
    fill = "PolitiFact Rating",
    title = "Claim Ratings in Debates"
  ) +
  scale_fill_manual(values = truenesscolors)
```

An unspecified debate is something like: a roundtable discussion, debates between politicians and a news anchor, or legislative debates.

An election debate is a debate between two candidates actively engaged in campaigns against each other.\
...
loc_tags %>%
  ...
  kable(caption = "Type of Debate by Party, count")
...
loc_tags %>%
 ...
 ggplot(aes(x = claimant_party, fill = textualRating)) +
  ...
  labs(
    ...
    y = "Rating Distribution",
    fill = "PolitiFact Rating",
    title = "Claim Ratings in Debates, by Political Party"
  ) +
  scale_fill_manual(values = truenesscolors)
```
